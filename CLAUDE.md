...
- Spring AI 1.1.4，对接 DeepSeek 与 Anthropic Claude
...
- `app`：应用服务层。
  ...
  - `AiChatServiceImpl` 根据 `model` 在 DeepSeek 与 Claude 的 `ChatClient` 之间选择。
- `infra`：基础设施配置。
  - `AiConfiguration` 创建两个命名 `ChatClient` Bean：`deepseek` 与 `claude`。
  - `WebConfig` 注册 UTF-8 编码过滤器。

### API 形态

- `POST /api/ai/chat`
  - 请求体：`{"message":"...","model":"deepseek|claude"}`
  - 返回：`Map<String,Object>`，成功时主要包含 `success: true` 与 `content`。
- `GET /api/ai/chat/stream?message=...&model=deepseek|claude`
  - 返回 `text/event-stream`。
  - 普通内容使用 `event: message`，结束时发送 `event: done`。

后端模型选择值应保持为 `deepseek` 或 `claude`；新增模型时需要同时更新后端 `AiChatServiceImpl` 的选择逻辑和前端模型选项。
